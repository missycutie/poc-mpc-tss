...
class Web3AuthMPCCoreKit {
  constructor(options) {
    ...
    defineProperty_default()(this, "preSigningHook", void 0);
  ...
  async loginWithOAuth(params) {
    ...
    if (registerExistingSFAKey && importTssKey) {
      throw errors.invalidConfig("Cannot import TSS key and register SFA key at the same time.");
    }
    if (this.isRedirectMode && (importTssKey || registerExistingSFAKey)) {
      throw errors.invalidConfig("key import is not supported in redirect mode");
    }
    try {
      // oAuth login.
      ...
      if (loginResponse && registerExistingSFAKey && loginResponse.finalKeyData.privKey) {
        if (loginResponse.metadata.typeOfUser === "v1") {
          throw errors.invalidConfig("Cannot register existing SFA key for v1 users, please contact web3auth support.");
        }
  ...
  async inputFactorKey(factorKey) {
    ...
    try {
      ...
      if (!this.tKey.secp256k1Key) {
        ...
        const factorKeyPrivate = tss_namespaceObject.factorKeyCurve.keyFromPrivate(factorKey.toBuffer());
        ...
        if (!factorEncExist) {
          throw errors.providedFactorKeyInvalid("Invalid FactorKey provided. Failed to input factor key.");
        }
  ...
  async setupTkey(providedImportKey, sfaLoginResponse, importingSFAKey) {
    if (importingSFAKey && !sfaLoginResponse) {
      throw errors["default"]("SFA key registration requires SFA login response");
    }
    if (!this.state.postBoxKey) {
      throw errors.userNotLoggedIn();
    }
    ...
    if (!existingUser) {
      ...
      if (importingSFAKey && sfaLoginResponse && sfaLoginResponse.metadata.upgraded) {
        throw errors["default"]("SFA key registration is not allowed for already upgraded users");
      }
    ...
    await this.atomicSync(async () => {
      ...
      if (importTssKey && isSfaKey) {
        await this.tkey.addLocalMetadataTransitions({
          ...
          privKey: [new (external_bn_js_default())(this.state.postBoxKey, "hex")]
        });
      }
    });
  }
