...
class Web3AuthMPCCoreKit {
  ...
  async loginWithOAuth(params) {
    ...
    if (registerExistingSFAKey && importTssKey) {
      throw errors.default.invalidConfig("Cannot import TSS key and register SFA key at the same time.");
    }
    if (this.isRedirectMode && (importTssKey || registerExistingSFAKey)) {
      throw errors.default.invalidConfig("key import is not supported in redirect mode");
    }
    try {
      // oAuth login.
      ...
      if (loginResponse && registerExistingSFAKey && loginResponse.finalKeyData.privKey) {
        if (loginResponse.metadata.typeOfUser === "v1") {
          throw errors.default.invalidConfig("Cannot register existing SFA key for v1 users, please contact web3auth support.");
        }
  ...
  async inputFactorKey(factorKey) {
    ...
    try {
      ...
      if (!this.tKey.secp256k1Key) {
        ...
        const factorKeyPrivate = tss.factorKeyCurve.keyFromPrivate(factorKey.toBuffer());
        ...
        if (!factorEncExist) {
          throw errors.default.providedFactorKeyInvalid("Invalid FactorKey provided. Failed to input factor key.");
        }
  ...
  async setupTkey(providedImportKey, sfaLoginResponse, importingSFAKey) {
    if (importingSFAKey && !sfaLoginResponse) {
      throw errors.default.default("SFA key registration requires SFA login response");
    }
    if (!this.state.postBoxKey) {
      throw errors.default.userNotLoggedIn();
    }
    ...
    if (!existingUser) {
      if (!importKey && this.useClientGeneratedTSSKey) {
        if (this.keyType === commonTypes.KeyType.ed25519) {
          ...
          importKey = utils.scalarBNToBufferSEC1(k).toString("hex");
        } else {
          throw errors.default.default("Unsupported key type");
        }
      }
      if (importingSFAKey && sfaLoginResponse && sfaLoginResponse.metadata.upgraded) {
        throw errors.default.default("SFA key registration is not allowed for already upgraded users");
      }
    ...
    await this.atomicSync(async () => {
      ...
      if (importTssKey && isSfaKey) {
        await this.tkey.addLocalMetadataTransitions({
          input: [{
            message: commonTypes.ONE_KEY_DELETE_NONCE
          }],
