...
var rssClient = require('@toruslabs/rss-client');
...
const factorKeyCurve = new elliptic.ec(FACTOR_KEY_TYPE);
const LEGACY_KEY_TYPE = "secp256k1";
class TKeyTSS extends core.TKey {
  /**
  ...
  constructor(args) {
    ...
    if (serviceProvider.customAuthArgs.keyType !== tssKeyType) {
      throw core.CoreError.default(`service provider keyType mismatch: ${serviceProvider.customAuthArgs.keyType} !== ${tssKeyType}`);
    }
    this.serviceProvider = serviceProvider;
    this.storageLayer = storageLayer;
    this._tssTag = tssTag;
    this._tssKeyType = tssKeyType;
    this._tssCurve = new elliptic.ec(tssKeyType);
  }
  get tssTag() {
    return this._tssTag;
  }
  ...
  set tssTag(tag) {
    if ((this.metadata.tssKeyTypes[this.tssTag] || LEGACY_KEY_TYPE) !== this.tssKeyType) {
      throw core.CoreError.default(`tssKeyType mismatch: ${this.metadata.tssKeyTypes[this.tssTag]} !== ${this.tssKeyType}`);
    }
    this._tssTag = tag;
  }
  ...
  async initialize(params) {
    ...
    if (!this.metadata.tssPolyCommits[this.tssTag] && !(params !== null && params !== void 0 && params.skipTssInit || params !== null && params !== void 0 && params.neverInitializeNewKey)) {
      ...
      const accountSalt = util.generateSalt(this._tssCurve);
  ...
  getFactorEncs(factorPub) {
    if (!this.metadata) throw core.CoreError.metadataUndefined();
    if (!this.metadata.factorEncs) throw core.CoreError.default("no factor encs mapping");
    if (!this.metadata.factorPubs) throw core.CoreError.default("no factor pubs mapping");
    const factorPubs = this.metadata.factorPubs[this.tssTag];
    if (!factorPubs) throw core.CoreError.default(`no factor pubs for this tssTag ${this.tssTag}`);
    if (factorPubs.filter(f => f.x.cmp(factorPub.x) === 0 && f.y.cmp(factorPub.y) === 0).length === 0) throw core.CoreError.default(`factor pub ${factorPub} not found for tssTag ${this.tssTag}`);
    if (!this.metadata.factorEncs[this.tssTag]) throw core.CoreError.default(`no factor encs for tssTag ${this.tssTag}`);
  ...
  async getTSSShare(factorKey, opts) {
    const factorPub = util.getPubKeyPoint(factorKey, factorKeyCurve);
    ...
    const userDecryption = await commonTypes.decrypt(Buffer.from(factorKey.toString(16, 64), "hex"), userEnc);
    const serverDecryptions = await Promise.all(serverEncs.map(factorEnc => {
      if (factorEnc === null) return null;
      return commonTypes.decrypt(Buffer.from(factorKey.toString(16, 64), "hex"), factorEnc);
    }));
    ...
    const combis = util.kCombinations(serverDecs.length, threshold);
    for (let i = 0; i < combis.length; i++) {
      ...
      const serverLagrangeCoeffs = selectedServerIndexes.map(x => util.getLagrangeCoeffs(ec, selectedServerIndexes, x));
      const serverInterpolated = rssClient.dotProduct(serverLagrangeCoeffs, selectedServerDecs, ec.n);
      const lagrangeCoeffs = [util.getLagrangeCoeffs(ec, [1, 99], 1), util.getLagrangeCoeffs(ec, [1, 99], 99)];
      const tssShare = rssClient.dotProduct(lagrangeCoeffs, [serverInterpolated, userDec], ec.n);
  ...
  getTSSCommits() {
    ...
    if (!tssPolyCommits) throw core.CoreError.default(`tss poly commits not found for tssTag ${this.tssTag}`);
    if (tssPolyCommits.length === 0) throw core.CoreError.default("tss poly commits is empty");
  ...
  getTSSPub(accountIndex) {
    ...
    if (accountIndex && accountIndex > 0) {
      ...
      return commonTypes.Point.fromElliptic(devicePubKeyPoint);
    }
    return tssCommits[0];
  }
  ...
  async importTssKey(params, serverOpts) {
    const ec = this._tssCurve;
    if (!this.secp256k1Key) throw core.CoreError.privateKeyUnavailable();
    if (!this.metadata) {
      throw core.CoreError.metadataUndefined();
    }
    const {
      importKey,
      factorPub,
    ...
    try {
      ...
      if (!tag) throw core.CoreError.default(`invalid param, tag is required`);
      if (!factorPub) throw core.CoreError.default(`invalid param, newFactorPub is required`);
      if (!newTSSIndex) throw core.CoreError.default(`invalid param, newTSSIndex is required`);
      if (authSignatures.length === 0) throw core.CoreError.default(`invalid param, authSignatures is required`);
      ...
      if ((existingFactorPubs === null || existingFactorPubs === void 0 ? void 0 : existingFactorPubs.length) > 0) {
        throw core.CoreError.default(`Duplicate account tag, please use a unique tag for importing key`);
      }
      const factorPubs = [factorPub];
      const importScalar = await (async () => {
        if (this._tssKeyType === commonTypes.KeyType.secp256k1) {
          return new BN(importKey);
        } else if (this._tssKeyType === commonTypes.KeyType.ed25519) {
          // Store seed in metadata.
          const domainKey = util.getEd25519SeedStoreDomainKey(this.tssTag || TSS_TAG_DEFAULT);
          ...
          const {
            scalar
          } = torus_js.getEd25519ExtendedPublicKey(importKey);
          const encKey = Buffer.from(torus_js.getSecpKeyFromEd25519(scalar).point.encodeCompressed("hex"), "hex");
          const msg = await commonTypes.encrypt(encKey, importKey);
      ...
      const tssPubKey = rssClient.hexPoint(ec.g.mul(importScalar));
      ...
      if ((nodeIndexes === null || nodeIndexes === void 0 ? void 0 : nodeIndexes.length) > 0) {
        ...
        finalSelectedServers = rssClient.randomSelection(new Array(rssNodeDetails.serverEndpoints.length).fill(null).map((_, i) => i + 1), Math.ceil(rssNodeDetails.serverEndpoints.length / 2));
      }
      const {
        serverEndpoints,
        serverPubKeys,
      ...
      const rssClient$1 = new rssClient.RSSClient({
        serverEndpoints,
        serverPubKeys,
        serverThreshold,
        tssPubKey,
      ...
      const refreshResponses = await rssClient$1.import({
        importKey: importScalar,
        dkgNewPub: util.pointToHex(newTSSServerPub),
        selectedServers: finalSelectedServers,
        factorPubs: factorPubs.map(f => util.pointToHex(f)),
        targetIndexes: tssIndexes,
        newLabel: label,
        sigs: authSignatures
      });
      const secondCommit = newTSSServerPub.toEllipticPoint(ec).add(rssClient.ecPoint(ec, tssPubKey).neg());
      const newTSSCommits = [commonTypes.Point.fromJSON(tssPubKey), commonTypes.Point.fromJSON({
  ...
  async _UNSAFE_exportTssKey(tssOptions) {
    if (!this.metadata) throw core.CoreError.metadataUndefined("metadata is undefined");
    ...
    const tempFactorPub = util.getPubKeyPoint(tempFactorKey, factorKeyCurve);
    ...
    const ec = this._tssCurve;
    const tssKey = util.lagrangeInterpolation(ec, [tempShare, factorShare], [new BN(tempIndex), new BN(factorIndex)]);
  ...
  async _UNSAFE_exportTssEd25519Seed(tssOptions) {
    ...
    const domainKey = util.getEd25519SeedStoreDomainKey(this.tssTag || TSS_TAG_DEFAULT);
    const result = this.metadata.getGeneralStoreDomain(domainKey);
    const decKey = torus_js.getSecpKeyFromEd25519(edScalar).scalar;
    const seed = await commonTypes.decrypt(decKey.toArrayLike(Buffer, "be", 32), result.message);
    return seed;
  }
  ...
  async _refreshTSSShares(updateMetadata, inputShare, inputIndex, factorPubs, targetIndexes, verifierNameVerifierId, serverOpts) {
    if (!this.metadata) throw core.CoreError.metadataUndefined();
    if (!this.metadata.tssPolyCommits) throw core.CoreError.default(`tss poly commits obj not found`);
    const tssCommits = this.metadata.tssPolyCommits[this.tssTag];
    if (!tssCommits) throw core.CoreError.default(`tss commits not found for tssTag ${this.tssTag}`);
    if (tssCommits.length === 0) throw core.CoreError.default(`tssCommits is empty`);
    const tssPubKeyPoint = tssCommits[0];
    const tssPubKey = util.pointToHex(tssPubKeyPoint);
    const {
      serverEndpoints,
      serverPubKeys,
      serverThreshold,
    ...
    if (!this.metadata.factorPubs) throw core.CoreError.default(`factorPubs obj not found`);
    if (!factorPubs) throw core.CoreError.default(`factorPubs not found for tssTag ${this.tssTag}`);
    if (factorPubs.length === 0) throw core.CoreError.default(`factorPubs is empty`);
    if (!this.metadata.tssNonces) throw core.CoreError.default(`tssNonces obj not found`);
    ...
    const refreshResponses = await rssClient$1.refresh({
      factorPubs: factorPubs.map(f => util.pointToHex(f)),
      targetIndexes,
      oldLabel,
      newLabel,
    ...
    const secondCommit = newTSSServerPub.toEllipticPoint(this._tssCurve).add(rssClient.ecPoint(this._tssCurve, tssPubKey).neg());
    const newTSSCommits = [commonTypes.Point.fromJSON(tssPubKey), commonTypes.Point.fromJSON({
      x: secondCommit.getX().toString(16, 64),
      y: secondCommit.getY().toString(16, 64)
    })];
  ...
  computeAccountNonce(index) {
    ...
    if (this._tssKeyType === commonTypes.KeyType.ed25519) {
    ...
    let accountHash = keccak.keccak256(Buffer.from(`${index}${this._accountSalt}`));
  ...
  async reconstructKey(_reconstructKeyMiddleware) {
    ...
    if (accountSalt && accountSalt.value) {
      ...
      const newSalt = util.generateSalt(this._tssCurve);
      await this._setTKeyStoreItem(TSS_MODULE, {
        id: "accountSalt",
        value: newSalt
      });
  ...
  async addFactorPub(args) {
    ...
    if (tssIndex !== newTSSIndex && !refreshShares) {
      throw core.CoreError.default("newTSSIndex does not match existing tssIndex, set refreshShares to true to refresh shares");
    }
    if (!refreshShares) {
      ...
      factorEncs[factorPubID] = {
        tssIndex,
        type: "direct",
        userEnc: await commonTypes.encrypt(newFactorPub.toSEC1(commonTypes.secp256k1, false), tssShare.toArrayLike(Buffer, "be", 32)),
        serverEncs: []
      };
      this.metadata.updateTSSData({
      ...
      const rssNodeDetails = await this._getRssNodeDetails();
      const randomSelectedServers = rssClient.randomSelection(new Array(rssNodeDetails.serverEndpoints.length).fill(null).map((_, i) => i + 1), Math.ceil(rssNodeDetails.serverEndpoints.length / 2));
  ...
  async deleteFactorPub(args) {
    ...
    if (found.length === 0) throw core.CoreError.default("could not find factorPub to delete");
    if (found.length > 1) throw core.CoreError.default("found two or more factorPubs that match, error in metadata");
  ...
  async _initializeNewTSSKey(tssTag, deviceTSSShare, factorPub, deviceTSSIndex) {
    ...
    const L1_0 = util.getLagrangeCoeffs(ec, [1, _tssIndex], 1, 0);
    const LIndex_0 = util.getLagrangeCoeffs(ec, [1, _tssIndex], _tssIndex, 0);
    const a0Pub = tss1PubKey.mul(L1_0).add(tss2PubKey.mul(LIndex_0));
    const a1Pub = tss1PubKey.add(a0Pub.neg());
    const tssPolyCommits = [commonTypes.Point.fromElliptic(a0Pub), commonTypes.Point.fromElliptic(a1Pub)];
    const factorPubs = [factorPub];
    const factorEncs = {};
    for (let i = 0; i < factorPubs.length; i++) {
      ...
      factorEncs[factorPubID] = {
        tssIndex: _tssIndex,
        type: "direct",
        userEnc: await commonTypes.encrypt(f.toSEC1(factorKeyCurve, false), Buffer.from(tss2.toString(16, 64), "hex")),
        serverEncs: []
      };
    }
    return {
...
exports.FACTOR_KEY_TYPE = FACTOR_KEY_TYPE;
exports.LEGACY_KEY_TYPE = LEGACY_KEY_TYPE;
exports.TKeyTSS = TKeyTSS;
exports.TSS_MODULE = TSS_MODULE;
exports.TSS_TAG_DEFAULT = TSS_TAG_DEFAULT;
exports.factorKeyCurve = factorKeyCurve;
