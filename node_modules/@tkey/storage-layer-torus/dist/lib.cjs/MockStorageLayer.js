...
class MockStorageLayer {
  ...
  async getMetadata(params) {
    ...
    let usedKey;
    if (!privKey) usedKey = serviceProvider.retrievePubKeyPoint().getX();else usedKey = commonTypes.getPubKeyPoint(privKey).x;
  ...
  async setMetadataStream(params) {
    ...
    input.forEach((el, index) => {
      let usedKey;
      if (!privKey || !privKey[index]) usedKey = serviceProvider.retrievePubKeyPoint().getX();else usedKey = commonTypes.getPubKeyPoint(privKey[index]).x;
  ...
  async acquireWriteLock(params) {
    ...
    const id = commonTypes.generateID();
...
module.exports = MockStorageLayer$1;
