...
const getTSSEndpoints = (sapphireNetwork, legacyNetwork, keyType = constants_namespaceObject.KEY_TYPE.SECP256K1) => {
  const endpoints = SAPPHIRE_NETWORK_URLS[sapphireNetwork];
  if (!endpoints || endpoints.length === 0) {
    throw new Error(`Unsupported network: ${sapphireNetwork}`);
  }
  const tssPath = keyType === constants_namespaceObject.KEY_TYPE.ED25519 ? "tss-frost" : "tss";
  const routeIdentifier = constants_namespaceObject.LEGACY_NETWORKS_ROUTE_MAP[legacyNetwork];
  return endpoints.map(e => {
    if (routeIdentifier && routeIdentifier.networkIdentifier) {
