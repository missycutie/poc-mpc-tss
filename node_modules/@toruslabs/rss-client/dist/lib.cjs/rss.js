...
function getEndpoint(endpoint, path, options_, customOptions) {
  if (typeof endpoint === "string") {
    return httpHelpers.get(`${endpoint}${path}`, options_, customOptions);
  }
  return endpoint.get(path);
}
function postEndpoint(endpoint, path, data, options_, customOptions) {
  if (typeof endpoint === "string") {
    return httpHelpers.post(`${endpoint}${path}`, data, options_, customOptions);
  }
...
class RSSClient {
  constructor(opts) {
    ...
    this.ecCurve = new elliptic.ec(this.keyType);
    this.tssPubKey = utils.ecPoint(this.ecCurve, opts.tssPubKey);
    ...
    if (opts.tempKey) {
      ...
      this.tempPubKey = utils.ecCurveSecp256k1.g.mul(opts.tempKey);
    } else {
      const kp = utils.ecCurveSecp256k1.genKeyPair();
  ...
  async import(opts) {
    ...
    const rssRound1Proms = selectedServers.map(ind => {
      ...
      return postEndpoint(serverEndpoint, "/rss_round_1", {
        ...
        user_temp_pubkey: utils.hexPoint(this.tempPubKey),
    ...
    const _finalLagrangeCoeffs = targetIndexes.map(target => utils.getLagrangeCoeff([0, 1], 0, target, this.ecCurve.n).umod(this.ecCurve.n));
    const _masterPolys = [];
    ...
    for (let i = 0; i < _finalLagrangeCoeffs.length; i++) {
      ...
      const _m = utils.generatePolynomial(1, _lc.mul(importKey).umod(this.ecCurve.n), generateRandomScalar);
      _masterPolys.push(_m);
      _masterPolyCommits.push(_m.map(coeff => {
        const _gCoeff = this.ecCurve.g.mul(coeff);
        return utils.hexPoint(_gCoeff);
      }));
      const _s = utils.generatePolynomial(serversInfo.threshold - 1, utils.getShare(_m, 1, this.ecCurve.n), generateRandomScalar);
      _serverPolys.push(_s);
      _serverPolyCommits.push(_s.map(coeff => utils.hexPoint(this.ecCurve.g.mul(coeff))));
    }
    const _serverEncs = [];
    const _userEncs = [];
    ...
    for (let i = 0; i < targetIndexes.length; i++) {
      ...
      _userEncs.push(await utils.encrypt(Buffer.from(`04${utils.hexPoint(this.tempPubKey).x.padStart(64, "0")}${utils.hexPoint(this.tempPubKey).y.padStart(64, "0")}`, "hex"), Buffer.from(utils.getShare(_masterPoly, 99, this.ecCurve.n).toString(16, 64), "hex")));
      ...
      for (let j = 0; j < serversInfo.pubkeys.length; j++) {
        ...
        _serverEnc.push(await utils.encrypt(Buffer.from(`04${_pub.x.padStart(64, "0")}${_pub.y.padStart(64, "0")}`, "hex"), Buffer.from(utils.getShare(_serverPoly, j + 1, this.ecCurve.n).toString(16, 64), "hex")));
      }
    }
    const _data = [];
    for (let i = 0; i < targetIndexes.length; i++) {
    ...
    const sums = targetIndexes.map((_, i) => {
      ...
      for (let j = 0; j < rssRound1Responses.length; j++) {
        ...
        if (sumMasterPolyCommits.length === 0 && sumServerPolyCommits.length === 0) {
          sumMasterPolyCommits = masterPolyCommits.map(p => utils.ecPoint(this.ecCurve, p));
          sumServerPolyCommits = serverPolyCommits.map(p => utils.ecPoint(this.ecCurve, p));
          continue;
        }
        sumMasterPolyCommits = sumMasterPolyCommits.map((summedCommit, k) => {
          return utils.ecPoint(this.ecCurve, masterPolyCommits[k]).add(summedCommit);
        });
        sumServerPolyCommits = sumServerPolyCommits.map((summedCommit, k) => {
          return utils.ecPoint(this.ecCurve, serverPolyCommits[k]).add(summedCommit);
        });
      }
      return {
    ...
    targetIndexes.map((target, i) => {
      ...
      const temp1 = utils.ecPoint(this.ecCurve, dkgNewPub).mul(utils.getLagrangeCoeff([1, target], 1, 0, this.ecCurve.n));
      const temp2 = mc[0].mul(utils.getLagrangeCoeff([1, target], target, 0, this.ecCurve.n));
    ...
    for (let i = 0; i < targetIndexes.length; i++) {
      ...
      const userDecs = await Promise.all(userEncs.map(encMsg => utils.decrypt(privKeyBuffer, encMsg)));
    ...
    const userFactorEncs = await Promise.all(userShares.map((userShare, i) => {
      ...
      return utils.encrypt(Buffer.from(`04${pub.x.padStart(64, "0")}${pub.y.padStart(64, "0")}`, "hex"), Buffer.from(userShare.toString(16, 64), "hex"));
    }));
    ...
    const serverFactorEncs = await Promise.all(serverIndexes.map(ind => {
      ...
      targetIndexes.map((_, i) => {
        ...
        const round2RequestData = {
          master_commits: mc.map(utils.hexPoint),
          server_commits: sc.map(utils.hexPoint),
  ...
  async refresh(opts) {
    ...
    // front end also generates hierarchical secret sharing
    // - calculate lagrange coeffs
    const _L = utils.getLagrangeCoeff([1, inputIndex], inputIndex, 0, this.ecCurve.n);
    const _finalLagrangeCoeffs = targetIndexes.map(target => _L.mul(utils.getLagrangeCoeff([0, 1], 0, target, this.ecCurve.n)).umod(this.ecCurve.n));
    const _masterPolys = [];
    const _masterPolyCommits = [];
    const _serverPolys = [];
    const _serverPolyCommits = [];
    ...
    for (let i = 0; i < _finalLagrangeCoeffs.length; i++) {
      const _lc = _finalLagrangeCoeffs[i];
      const _m = utils.generatePolynomial(1, _lc.mul(inputShare).umod(this.ecCurve.n), generateRandomScalar);
      _masterPolys.push(_m);
      _masterPolyCommits.push(_m.map(coeff => {
        const _gCoeff = this.ecCurve.g.mul(coeff);
        return utils.hexPoint(_gCoeff);
...
async function recover(opts) {
  ...
  if (opts.keyType !== "secp256k1" && opts.keyType !== "ed25519") throw new Error("Invalid keyType, only secp256k1 or ed25519 is supported");
  const ecCurve = new elliptic.ec(keyType);
  const factorKeyBuf = Buffer.from(factorKey.toString(16, 64), "hex");
  const prom1 = utils.decrypt(factorKeyBuf, userEnc).then(buf => new BN(buf));
  const prom2 = Promise.all(serverEncs.map(serverEnc => serverEnc && utils.decrypt(factorKeyBuf, serverEnc).then(buf => new BN(buf))));
  ...
  const decryptedLCs = selectedServers.map(index => utils.getLagrangeCoeff(selectedServers, index, 0, ecCurve.n));
  const temp1 = decryptedUserEnc.mul(utils.getLagrangeCoeff([1, 99], 99, 0, ecCurve.n));
  const serverReconstructed = utils.dotProduct(someDecrypted, decryptedLCs).umod(ecCurve.n);
  const temp2 = serverReconstructed.mul(utils.getLagrangeCoeff([1, 99], 1, 0, ecCurve.n));
