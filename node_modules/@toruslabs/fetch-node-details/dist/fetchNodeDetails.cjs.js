...
class NodeDetailManager {
  constructor({
    ...
    keyType = constants_namespaceObject.KEY_TYPE.SECP256K1,
    fndServerEndpoint,
    enableLogging = false
  } = {}) {
  ...
  async getNodeDetails({
    ...
    try {
      ...
      try {
        const {
          nodeDetails
        } = await (0,http_helpers_namespaceObject.get)(`${this.fndServerEndpoint}?network=${this.network}&verifier=${verifier}&verifierId=${verifierId}&keyType=${this._keyType}`);
        this.setNodeDetails(nodeDetails);
        return this._nodeDetails;
      } catch (error) {
        log.error("Failed to fetch node details from server, using local.", error);
      }
      const nodeDetails = (0,fnd_base_namespaceObject.fetchLocalConfig)(this.network, this._keyType);
