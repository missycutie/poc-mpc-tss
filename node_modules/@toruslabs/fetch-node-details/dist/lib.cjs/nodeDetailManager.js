  ...
  async getNodeDetails({
    ...
    try {
      ...
      try {
        const {
          nodeDetails
        } = await httpHelpers.get(`${this.fndServerEndpoint}?network=${this.network}&verifier=${verifier}&verifierId=${verifierId}&keyType=${this._keyType}`);
        this.setNodeDetails(nodeDetails);
        return this._nodeDetails;
      } catch (error) {
        log.error("Failed to fetch node details from server, using local.", error);
      }
      const nodeDetails = fndBase.fetchLocalConfig(this.network, this._keyType);
...
module.exports = NodeDetailManager$1;
