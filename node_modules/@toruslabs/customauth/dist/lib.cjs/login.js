...
class CustomAuth {
    ...
    if (hash && queryParameters) {
      ...
      if (this.config.uxMode === enums.UX_MODE.REDIRECT) {
        await this.storageHelper.storeLoginDetails({
          method: enums.TORUS_METHOD.TRIGGER_LOGIN,
          args
        }, loginHandler.nonce);
      }
  ...
  async triggerAggregateLogin(args) {
    ...
    if (aggregateVerifierType === enums.AGGREGATE_VERIFIER.SINGLE_VERIFIER_ID && subVerifierDetailsArray.length !== 1) {
    ...
    for (const subVerifierDetail of subVerifierDetailsArray) {
      ...
      if (hash && queryParameters) {
        ...
        if (this.config.uxMode === enums.UX_MODE.REDIRECT) {
          await this.storageHelper.storeLoginDetails({
            method: enums.TORUS_METHOD.TRIGGER_AGGREGATE_LOGIN,
            args
          }, loginHandler.nonce);
        }
        loginParams = await loginHandler.handleLoginWindow({
    ...
    const aggregateIdToken = torus_js.keccak256(Buffer.from(aggregateIdTokenSeeds.join(String.fromCharCode(29)), "utf8")).slice(2);
